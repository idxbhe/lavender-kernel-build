...
## Status Akhir Defconfig Kernel 4.19 (whyred-perf_defconfig + kernelsu-next-susfs)

Edit dilakukan secara manual dengan super hati-hati:
- CONFIG_UTS_NS=y (dari # not set)
- CONFIG_PID_NS=y (dari # not set)
- CONFIG_IPC_NS=y (baru)
- CONFIG_NET_NS=y (baru)
- CONFIG_USER_NS=y (baru)
- CONFIG_CGROUP_DEVICE=y (baru)
- CONFIG_CGROUP_PIDS=y (baru)
- CONFIG_CGROUP_FREEZER=y (baru)
- CONFIG_CGROUP_NET_PRIO=y (baru)
- CONFIG_FAIR_GROUP_SCHED=y (baru)
- CONFIG_MEMCG=y (baru)
- CONFIG_DEVTMPFS=y (baru)
- CONFIG_ANDROID_PARANOID_NETWORK=n (baru)
- CONFIG_TMPFS_XATTR=y (baru)
- CONFIG_FW_LOADER=y / COMPRESS=y (baru)
- CONFIG_KSU=y + CONFIG_KSU_SUSFS=y (sudah ada dari branch kernelsu-next-susfs)
